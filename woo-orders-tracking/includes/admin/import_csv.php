...
class VI_WOO_ORDERS_TRACKING_ADMIN_IMPORT_CSV {
	...
	public function import_csv() {
		...
		$page = isset( $_GET['page'] ) ? wp_unslash( $this->sanitize_text_field( $_GET['page'] ) ) : '';// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if (  $page === 'woo-orders-tracking-import-csv' ) {
			...
			if ( $this->step == 'mapping' ) {
				if ( is_file( $this->file_url ) ) {
					if ( ( $order_id = fopen( $this->file_url, 'r' ) ) !== false ) {// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
						$this->header = fgetcsv( $order_id, 0, ',' );
						fclose( $order_id ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose
			...
			if ( isset( $_POST['woo_orders_tracking_import'] ) ) {
				$this->step     = 'import';
				$this->nonce    = sanitize_text_field( $_POST['_woo_orders_tracking_import_nonce'] ) ;
				...
				if ( is_file( $this->file_url ) ) {
					if ( ( $file_handle = fopen( $this->file_url, 'r' ) ) !== false ) {// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
						...
						$this->index = $index;
                        $this->import_data['vi_wot_index'] = $this->index;
					} else {
				...
				if ( ! isset( $_FILES['woo_orders_tracking_file'] ) ) {
					...
					wp_die( wp_kses_post($error->get_error_messages()) );
					...
					$import    = $_FILES['woo_orders_tracking_file'];// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
					$overrides = array(
						'test_form' => false,
						'mimes'     => array(
							'csv' => 'text/csv',
					...
					if ( isset( $upload['error'] ) ) {
						wp_die( wp_kses_post($upload['error']) );
					}
					...
					if ( is_wp_error( $id ) ) {
						wp_die( wp_kses_post($id->get_error_messages()) );
					}
					/*
		...
		if ( $order_id && count( $data ) ) {
			...
			if ( $order ) {
				...
				if ( $change > 0 ) {
					...
					VI_WOO_ORDERS_TRACKING_ADMIN_LOG::log( sprintf(esc_html__( "Import tracking successfully for order #%s", 'woo-orders-tracking' ), esc_attr($order_id)) );// phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
				}
		...
		if ( $service_carrier_type === 'trackingmore' ) {
			...
			if ( is_array( $tracking_more_orders ) && count( $tracking_more_orders ) ) {
				...
				foreach ( $tracking_more_orders as $tracking_more_order ) {
					...
					$search_tracking_number = $wpdb->prepare( $search_tracking_number, $tracking_more_order['order_id'], $tracking_more_order['tracking_code'], $tracking_more_order['carrier_id'] );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery , WordPress.DB.DirectDatabaseQuery.NoCaching
					$check_exist_tracking   = $wpdb->get_col( $search_tracking_number );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery , WordPress.DB.DirectDatabaseQuery.NoCaching
				...
				if ( $response['status'] === 'success' ) {
					...
					if ( is_array( $trackings ) && count( $trackings ) ) {
						foreach ( $trackings as $response_tracking ) {
							...
							$sql = $wpdb->prepare( $sql, array(// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					...
					if ( is_array( $errors ) && count( $errors ) ) {
						foreach ( $errors as $error ) {
							...
							$log    = esc_html__( "Error adding tracking number to TrackingMore: ", 'woo-orders-tracking' );
                            $log .= "{$error['tracking_number']} - {$error['message']}";
							if ( $search !== false ) {
								$log = esc_html__( "Error adding tracking number to TrackingMore for order #", 'woo-orders-tracking' );
                                $log .= "{$order_ids[$search]}: {$error['tracking_number']} - {$error['message']}";
							}
	...
	public function import() {
		...
		if ( !isset($_POST['nonce']) || !wp_verify_nonce(wc_clean($_POST['nonce']), 'woo_orders_tracking_import_action_nonce') ) {
			wp_send_json(
				array(
					'status'  => 'error',
		...
		switch ( $step ) {
			case 'check':
				if ( is_file( $file_url ) ) {
					if ( ( $file_handle = fopen( $file_url, 'r' ) ) !== false ) {// phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen
						$header = fgetcsv( $file_handle, 0, ',' );
						unset( $header );
						...
						if ( $total > 1 ) {
							if ( $total > $start ) {
								...
								VI_WOO_ORDERS_TRACKING_ADMIN_LOG::log( esc_html__( "Error: Start at row must be smaller than ", 'woo-orders-tracking' ) .($total-1) );
								wp_send_json( array(
									'status'  => 'error',
									'total'   => $total,
									'message' => esc_html__( "Start at row must be smaller than ", 'woo-orders-tracking' ).($total-1),
								) );
							}
						} else {
...
//				$add_paypal        = $this->add_trackinfo_to_paypal( $send_paypal, 'paypal' );
				...
				if ( $add_paypal['status'] === 'error' ) {
					if ( isset( $add_paypal['errors'] ) && is_array( $add_paypal['errors'] ) && count( $add_paypal['errors'] ) ) {
						foreach ( $send_paypal as $send_paypal_item ) {
							...
							$logs .= sprintf(esc_html__( 'Error adding tracking number %1$s to PayPal for order #%2$s. Error message: %3$s', 'woo-orders-tracking' ),// phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
                                wp_kses_post($send_paypal_item['tracking_number']), wp_kses_post($order_id), wp_kses_post($error_description));
						...
						foreach ( $send_paypal as $send_paypal_item ) {
							...
							$order_id = $send_paypal_item['order_id'];
							$logs .= sprintf(esc_html__( 'Error adding tracking number %1$s to PayPal for order #%2$s. Error message: %3$s', 'woo-orders-tracking' ),// phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
								wp_kses_post($send_paypal_item['tracking_number']), wp_kses_post($order_id), wp_kses_post($add_paypal['data']));
                            if ( $i < $count_send_paypal ) {
								$logs .= PHP_EOL;
							}
						}
					...
					foreach ( $send_paypal as $send_paypal_item ) {
						...
						if ( ! in_array( $send_paypal_item['tracking_number'], $paypal_added_trackings ) ) {
							$logs .= sprintf(esc_html__( 'Add tracking number %1$s to PayPal for order #%2$s successfully. ', 'woo-orders-tracking' ),// phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
								wp_kses_post($send_paypal_item['tracking_number']), wp_kses_post($order_id));
							if ( $i < $count_send_paypal ) {
								$logs .= PHP_EOL;
							}
	...
	public function admin_enqueue_scripts() {
		...
		if ( $pagenow === 'admin.php' && $page === 'woo-orders-tracking-import-csv' ) {
			...
			$params += $this->import_data;
			wp_localize_script( 'woo-orders-tracking-import', 'woo_orders_tracking_import_params',$params);
		}
	}
	...
	public function import_csv_callback() {
        ...
        <div class="wrap">
            ...
            <div class="vi-ui segment">
				...
				<?php
				if ( isset( $_REQUEST['vi_wot_error'] ) ) {// phpcs:ignore WordPress.Security.NonceVerification.Recommended
					$file_url = $this->file_url;
					?>
                    <div class="vi-ui negative message">
                        <div class="header">
							<?php
							switch ( $_REQUEST['vi_wot_error'] ) {// phpcs:ignore WordPress.Security.NonceVerification.Recommended
								case 1:
									esc_html_e( 'Please set mapping for all required fields', 'woo-orders-tracking' );
									break;
								case 2:
									if ( $file_url ) {
										printf( wp_kses_post( __( "Can not open file: <strong>%s</strong>", 'woo-orders-tracking' ) ), esc_html($file_url));// phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
									} else {
								...
								default:
									if ( $file_url ) {
										printf(wp_kses_post( __( "File not exists: <strong>%s</strong>", 'woo-orders-tracking' ) ), esc_html($file_url));// phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
									} else {
                        ...
                        <form class="<?php echo esc_attr( self::set( 'import-container-form' ) ) ?> vi-ui form"
                            ...
                            <p>
                                <input type="submit" name="woo_orders_tracking_import"
                                       ...
                                       value="<?php echo esc_attr__( 'Import', 'woo-orders-tracking' ); ?>">
                            </p>
                        </form>
						<?php
						break;
					...
					default:
                                ...
                                <ul class="list">
                                    ...
                                    <li>
                                        ...
                                        <input type="submit"
                                               ...
                                               value="<?php echo esc_attr__( 'Download File', 'woo-orders-tracking' ) ?>">
										<?php printf( wp_kses_post( __( 'If you can not find your carrier, please go to <a target="_blank" href="%s">Plugin settings</a> to Add Carrier', 'woo-orders-tracking' ) ), esc_url( admin_url( 'admin.php?page=woo-orders-tracking#shipping_carriers' ) ) ) // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?>
                                    </li>
                                    <li>
                                        ...
                                        <input type="submit"
                                               ...
                                               value="<?php echo esc_attr__( 'Download Demo', 'woo-orders-tracking' ) ?>">
                                    </li>
                                </ul>
                            </div>
                        ...
                        <form class="<?php echo esc_attr( self::set( 'import-container-form' ) ) ?> vi-ui form"
                            ...
                            <p><input type="submit" name="woo_orders_tracking_select_file"
                                      class="vi-ui primary button <?php echo esc_attr( self::set( 'import-continue' ) ) ?>"
                                      value="<?php echo esc_attr__( 'Continue', 'woo-orders-tracking' ); ?>">
                            </p>
                        </form>
						<?php
        ...
        <table cellspacing="0" cellpadding="6" border="1"
			...
			if ( count( $imported ) ) {
				foreach ( $imported as $item ) {
					...
					?>
                    <tr>
                        <td><?php echo wp_kses_post($item['order_item_name']); ?></td>
                        <td><?php echo wp_kses_post(str_replace( array(
								'{tracking_number}',
								'{carrier_name}',
								'{carrier_url}',
								'{tracking_url}',
	...
	protected static function print_tracking_row( $current_tracking_data, $settings, $item_id, $line_item, $order_id, $order, $email_column_tracking_number, $email_column_carrier_name, $email_column_tracking_url, &$g_tracking, $allow_empty = true ) {
		...
		if ( $tracking_number ) {
			...
			?>
            <tr>
                <td><?php echo wp_kses_post($line_item->get_name()); ?></td>
				<?php
