...
class VI_WOO_ORDERS_TRACKING_ADMIN_ORDERS_EDIT_TRACKING {
			...
			if ( $order ) {
				...
				if ( $transID && $paypal_method && $item_tracking_data ) {
					...
					if ( $current_tracking_data['tracking_number'] ) {
						$response['message_content'] = '<div>' . sprintf( esc_html__( 'Tracking number: %s', 'woo-orders-tracking' ), $current_tracking_data['tracking_number'] ) . '</div>'; // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
					}
	...
	public function orders_tracking_edit_tracking_footer() {
		$order_id = isset( $_REQUEST['post'] ) ? wp_unslash( $_REQUEST['post'] ) : '';// phpcs:ignore WordPress.Security.NonceVerification.Recommended
        if (!$order_id && isset($_GET['page'], $_GET['id']) && sanitize_text_field(wp_unslash($_GET['page'])) === 'wc-orders'){// phpcs:ignore WordPress.Security.NonceVerification.Recommended
            ...
            <div class="<?php echo esc_attr( self::set( 'container' ) ) ?>">
                <div class="<?php echo esc_attr( self::set( 'item-details' ) ) ?>">
                    ...
                    <div class="<?php echo esc_attr( self::set( 'item-tracking-code-value' ) ) ?>"
                         title="<?php echo sprintf(esc_attr__( "Tracking carrier %s", 'woo-orders-tracking' ), esc_attr($carrier_name))  // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?>">
	...
	public function add_meta_box_callback() {
		...
		if (!$order) {
			$page     = isset( $_GET['page'] ) ? wc_clean( wp_unslash( $_GET['page'] ) ) : '';// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$order_id = $page === 'wc-orders' && ! empty( $_GET['id'] ) ? wc_clean( wp_unslash( $_GET['id'] ) ) : 0;// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			if ( ! $order_id ) {
				global $post;
