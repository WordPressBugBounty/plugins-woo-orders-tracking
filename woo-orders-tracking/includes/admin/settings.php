...
class VI_WOO_ORDERS_TRACKING_ADMIN_SETTINGS {
	...
	private function shipping_carriers_settings() {
        ...
        <div class="<?php echo esc_attr( self::set( array(
            ...
            <p><?php printf( wp_kses_post(__( 'Unable to find your needed shipping carrier? Please <a class="%s" href="#">click here</a> to add your own carrier.', 'woo-orders-tracking' )), esc_attr( self::set( array( 'setting-shipping-carriers-add-new-carrier-shortcut' ) ) ) ) // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?></p>
			...
			if ( $orders ) {
				...
				if ( count( $orders ) ) {
                    ...
                    <div class="vi-ui positive message">
                        <div class="header">
							<?php printf( wp_kses_post( __( 'Next schedule: <strong>%s</strong>', 'woo-orders-tracking' ) ), wp_kses_post(date_i18n( 'F j, Y g:i:s A', ( $this->schedule_send_emails + HOUR_IN_SECONDS * $gmt_offset ) )) );  // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?>
                        </div>
                        <p><?php printf( esc_html__( 'Order(s) to send next: %s', 'woo-orders-tracking' ), wp_kses_post(implode( ',', array_splice( $orders, 0, $this->settings->get_params( 'email_number_send' ) ) )) ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?></p>
                    </div>
					<?php
				}
			}
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td>
                    ...
                    <p class="description"><?php echo wp_kses_post(__( '<strong>*Note:</strong> If you use an email customizer plugin to send email, this option will be skipped. Tracking info will be included in all emails that <strong>contain order table</strong>.', 'woo-orders-tracking') ) ?></p>
                </td>
            </tr>
            <tr>
                <th>
                    ...
                    <select name="<?php echo esc_attr( self::set( 'settings' ) ) ?>[email_woo][email_woo_position]"
						...
						foreach ( $email_woo_positions as $email_woo_position_k => $email_woo_position_v ) {
							if ( $email_woo_position_k !== 'after_order_item' ) {
                                printf('<option value="%s" %s>%s</option>',esc_attr( $email_woo_position_k ),
                                    wp_kses_post($email_woo_position_k == $email_woo_position ? 'selected':''), esc_html( $email_woo_position_v ));
							} else {
								?>
            ...
            <tr>
                <td colspan="2">
                    <div class="vi-ui positive small message">
                        <div>
							<?php echo wp_kses_post(__( 'You can customize tracking number and carrier html which are displayed <strong>after every order item</strong> in email(if selected) or on <strong>My account/Order details</strong> page', 'woo-orders-tracking' )) ?>
                        </div>
                    </div>
                </td>
            </tr>
	...
	private function paypal_settings() {
		...
		if ( is_array( $available_paypal_methods ) && count( $available_paypal_methods ) ) {
            ...
            <div class="vi-ui positive message">
                ...
                <ul class="list">
                    <li><?php printf( wp_kses_post( __( 'Go to %s and login with your PayPal account', 'woo-orders-tracking' ) ), '<strong><a href="https://developer.paypal.com/developer/applications/" target="_blank">PayPal Developer</a></strong>' );  // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?></li>
                    ...
                    <tr class="wot-paypal-app-content">
                            ...
                            <div class="field">
                                <div class="field  woo-orders-tracking-setting-paypal-live-wrap">
                                    <div class="vi-ui input"
                                         data-tooltip="<?php echo esc_attr__( 'Live Client ID', 'woo-orders-tracking' ) ?>">
                                        <input type="text"
                                ...
                                <div class="field woo-orders-tracking-setting-paypal-sandbox-wrap">
                                    <div class="vi-ui input"
                                         data-tooltip="<?php echo esc_attr__( 'Sandbox Client ID', 'woo-orders-tracking' ) ?>">
                                        <input type="text"
                        ...
                        <td>
                            <div class="field ">
                                <div class="field  woo-orders-tracking-setting-paypal-live-wrap">
                                    <div class="vi-ui input"
                                         data-tooltip="<?php echo esc_attr__( 'Live Client Secret', 'woo-orders-tracking' ) ?>">
                                        <input type="text"
                                ...
                                <div class="field woo-orders-tracking-setting-paypal-sandbox-wrap">
                                    <div class="vi-ui input"
                                         data-tooltip="<?php echo esc_attr__( 'Sandbox Client Secret', 'woo-orders-tracking' ) ?>">
                                        <input type="text"
	...
	private function tracking_service_settings() {
        ...
        <div class="vi-ui positive small message">
            ...
            <ul class="list">
                <li><?php echo VI_WOO_ORDERS_TRACKING_DATA::wp_kses_post( sprintf( __( 'Branded tracking form shortcode %s. This only works if you use a tracking service. By default, tracking form is appended to the tracking page. If you want to change the tracking form\'s position, especially when the tracking page is built with an UX builder, this shortcode is very helpful.', 'woo-orders-tracking' ), '<span data-tooltip="Click to copy"><input type="text" class="wot-input-shortcode-field" readonly value="[vi_wot_form_track_order]"></span>' ) )  // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment , WordPress.Security.EscapeOutput.OutputNotEscaped ?></li>
                <li><?php echo VI_WOO_ORDERS_TRACKING_DATA::wp_kses_post( sprintf( __( 'TrackingMore tracking form shortcode %s. You can still use this shortcode even if you do not use tracking service. More details about this at <a target="_blank" href="https://www.trackingmore.com/embed_box_float-en.html">Track Button</a>', 'woo-orders-tracking' ), '<span data-tooltip="Click to copy"><input type="text" class="wot-input-shortcode-field" readonly value="[vi_wot_tracking_more_form]"></span>' ) )  // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment , WordPress.Security.EscapeOutput.OutputNotEscaped ?></li>
            </ul>
        </div>
        <table class="form-table">
            <tbody>
                    ...
                    <td>
                        ...
                        <select class="vi-ui dropdown"
							...
							foreach ( $statuses as $status_k => $status_v ) {
								?>
                                <option value="<?php echo esc_attr( $status_k ) ?>" <?php selected( $status_k, $ft_message['status'] ) ?>><?php echo wp_kses_post($status_v) ?></option>
								<?php
							}
							?>
                        </select>
	...
	public function wotv_admin_delete_shipping_carrier() {
		...
		if ( ! isset( $_POST['action_nonce'] ) || ! wp_verify_nonce( wc_clean($_POST['action_nonce']), 'vi_wot_setting_action_nonce' ) ) {
			return;
		}
	...
	public function preview_emails_button( $editor_id ) {
		global $pagenow;
		if ( $pagenow == 'admin.php' && isset( $_REQUEST['page'] ) && sanitize_text_field( $_REQUEST['page'] ) == 'woo-orders-tracking' ) {// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function wot_preview_emails() {
		...
		$heading                      = isset( $_GET['heading'] ) ?self::stripslashes_deep( stripslashes( $_GET['heading'] ) ) : '';
	...
	public function wot_test_connection_paypal() {
		...
		if ( $secret && $sandbox && $client_id ) {
			...
			if ( $check_token['status'] === 'success' ) {
				...
				$message = '<p class="' . esc_attr( self::set( 'error' ) ) . '">' . wp_kses_post($check_token['data']) . '</p>';
			}
			wp_send_json(
				array(
					'message' => $message
	...
	public function admin_enqueue_script() {
		...
		if ( $pagenow === 'admin.php' && $page === 'woo-orders-tracking' ) {
			...
			wp_enqueue_style( 'vi-wot-admin-setting-css', VI_WOO_ORDERS_TRACKING_CSS . 'admin-setting' . $src_min . '.css', '', VI_WOO_ORDERS_TRACKING_VERSION, false );
			wp_enqueue_style( 'vi-wot-admin-setting-support', VI_WOO_ORDERS_TRACKING_CSS . 'villatheme-support' . $src_min . '.css', '', VI_WOO_ORDERS_TRACKING_VERSION, false );
			...
			wp_enqueue_script( 'vi-wot-admin-setting-carrier-functions-js', VI_WOO_ORDERS_TRACKING_JS . '/carrier-functions' . $src_min . '.js', array( 'jquery' ), VI_WOO_ORDERS_TRACKING_VERSION , false);
			...
			wp_enqueue_script( 'vi-wot-admin-setting-js', VI_WOO_ORDERS_TRACKING_JS . 'admin-setting' . $src_min . '.js', array( 'jquery' ), VI_WOO_ORDERS_TRACKING_VERSION , false);
			$countries                = new WC_Countries();
