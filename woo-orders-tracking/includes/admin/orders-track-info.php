...
class VI_WOO_ORDERS_TRACKING_ADMIN_ORDERS_TRACK_INFO {
	...
	public function admin_enqueue_script() {
		...
		if ( ($pagenow === 'edit.php' && $post_type === 'shop_order') || (isset($_GET['page']) && wc_clean(wp_unslash($_GET['page'])) === 'wc-orders') ) {// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			wp_enqueue_style( 'vi-wot-admin-order-manager-icon', VI_WOO_ORDERS_TRACKING_CSS . 'woo-orders-tracking-icons' . $src_min . '.css', '', VI_WOO_ORDERS_TRACKING_VERSION );
			wp_enqueue_style( 'vi-wot-admin-order-manager-css', VI_WOO_ORDERS_TRACKING_CSS . 'admin-order-manager' . $src_min . '.css', '', VI_WOO_ORDERS_TRACKING_VERSION );
			...
			wp_enqueue_script( 'vi-wot-admin-order-manager-js', VI_WOO_ORDERS_TRACKING_JS . 'admin-order-manager' . $src_min . '.js', array( 'jquery' ), VI_WOO_ORDERS_TRACKING_VERSION, false );
			wp_localize_script(
            ...
            wp_add_inline_script('vi-wot-admin-order-manager-js',$this->addCustomImportButton());
		}
	}
	...
	public function addCustomImportButton() {
        ...
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                'use strict';
                $(".wrap .page-title-action").eq(0).after("<a class='page-title-action' target='_blank' href='<?php echo esc_url( admin_url( 'admin.php?page=woo-orders-tracking-import-csv' ) ); ?>'><?php esc_html_e( 'Import tracking number', 'woo-orders-tracking' ) ?></a>"
	...
	public function tracking_service_action_buttons_html( $tracking_link, $current_tracking_data, $tracking_status ) {
		...
		if ( ! empty( $current_tracking_data['last_update'] ) ) {
			$button_refresh_title = sprintf( esc_html__( 'Last update: %s. Click to refresh.', 'woo-orders-tracking' ), date_i18n( 'Y-m-d H:i:s', $current_tracking_data['last_update'] ) ); // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
		}
		...
		if ( $tracking_number && ! in_array( $tracking_number, $tracking_list ) ) {
            ...
            <div class="<?php echo esc_attr( self::set( $container_class ) ) ?>"
                ...
                <a class="<?php echo esc_attr( self::set( 'tracking-number' ) ) ?>"
                   ...
                   title="<?php printf(esc_attr__( "Tracking carrier %s", 'woo-orders-tracking' ), esc_attr($carrier_name))  // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?>"
	...
	public function refresh_track_info() {
		...
		$response        = array(
			...
			'button_title'             => sprintf( esc_html__( 'Last update: %s. Click to refresh.', 'woo-orders-tracking' ), date_i18n( 'Y-m-d H:i:s', time() ) ), // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
		);
		...
		if ( $order && $tracking_number && $carrier_slug && self::$settings->get_params( 'service_carrier_enable' ) ) {
			...
			if ( is_array( $carrier ) && count( $carrier ) ) {
				...
				$response['message_content'] .= '<div>' . sprintf( esc_html__( 'Carrier: %s', 'woo-orders-tracking' ), $carrier_name ) . '</div>'; // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
				$service_carrier_type        = self::$settings->get_params( 'service_carrier_type' );
	...
	public function restrict_manage_posts() {
		global $typenow;
		if ( in_array( $typenow, wc_get_order_types( 'view-orders' ), true ) || (wc_clean(wp_unslash($_GET['page'] ??'')) === 'wc-orders') ) {// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			?>
            <input type="text" name="woo_orders_tracking_search_tracking"
                   ...
                   autocomplete="off"
                   value="<?php echo isset( $_GET['woo_orders_tracking_search_tracking'] ) ? esc_attr( htmlentities( sanitize_text_field( $_GET['woo_orders_tracking_search_tracking'] ) ) ) : '' // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>">
			<?php
		}
	}
	...
	public function posts_where( $where, $wp_query ) {
		...
		$tracking_code = isset( $_GET['woo_orders_tracking_search_tracking'] ) ? wc_clean($_GET['woo_orders_tracking_search_tracking']) : '';// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( isset( $_GET['filter_action'] ) && $tracking_code && $post_type === 'shop_order' ) {// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function add_items_query( $args ) {
		if ( isset($_GET['page'] ) &&  sanitize_text_field(wp_unslash($_GET['page'])) === 'wc-orders' && !empty($_GET['woo_orders_tracking_search_tracking']) ) {// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$tracking_code = sanitize_text_field(wp_unslash($_GET['woo_orders_tracking_search_tracking']));// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			global $wpdb;
