...
class VI_WOO_ORDERS_TRACKING_ADMIN_EXPORT_ORDERS_TRACKING {
												...
												if ( isset( $item_data ) && is_array( $item_data ) && count( $item_data ) >= 2 ) {
													...
													if ( $item_data_id === '_billing_country' && isset( $billing_country ) && is_array( $billing_country ) && count( $billing_country ) ) {
                                                        ...
                                                        <option value="<?php echo esc_attr( $item ) ?>"
                                                                selected><?php printf(esc_html__( 'Country %$1s %2$s' , 'woo-orders-tracking' ), esc_html($t), wp_kses_post($item_name)) // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?></option>
														<?php
													} elseif ( $item_data_id === '_billing_city' && isset( $billing_city ) && is_array( $billing_city ) && count( $billing_city ) ) {
														?>
                                                        <option value="<?php echo esc_attr( $item ) ?>"
                                                                selected><?php printf(esc_html__( 'City %1$s %2$s', 'woo-orders-tracking' ),esc_html($t), wp_kses_post($item_data_name)) // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?></option>
														<?php
													}
												}
											}
											...
											foreach ( $export_settings_filter_shipping_address as $item ) {
												...
												if ( isset( $item_data ) && is_array( $item_data ) && count( $item_data ) >= 2 ) {
													...
													if ( $item_data_id === '_shipping_country' && isset( $shipping_country ) && is_array( $shipping_country ) && count( $shipping_country ) ) {
														...
														?>
                                                        <option value="<?php echo esc_attr( $item ) ?>"
                                                                selected><?php printf(esc_html__( 'Country %1$s %2$s', 'woo-orders-tracking' ) , esc_html($t), wp_kses_post($item_name)) // phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment ?></option>
														<?php
	...
	private function get_order_meta_values( $key ) {
		...
		$results = $wpdb->get_col( $query );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared , WordPress.DB.DirectDatabaseQuery.DirectQuery , WordPress.DB.DirectDatabaseQuery.NoCaching
		sort( $results );
	...
	public function vi_wot_export_preview() {
		...
		$settings = isset( $_POST['export_settings'] ) ? $this->stripslashes_deep( $_POST['export_settings'] ) : '';
		if ( empty( $settings ) ) {
			wp_send_json(
				array(
					'status'  => 'error',
					'message' => esc_html__( 'Miss field', 'woo-orders-tracking' )
				)
			);
			return;
		}

		$settings = vi_wot_json_decode( $settings );
